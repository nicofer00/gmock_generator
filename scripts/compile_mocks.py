...
def main(project_directory, output_directory, filepaths):
    ...
    for script in find_files(project_directory, VENV_SCRIPT):
        ...
        venv_script_path = ". " + venv_script_path if os.name == 'posix' else venv_script_path
                    ...
                    for found_file in find_files(project_directory, filename):
                        ...
                        if not os.path.exists(mock_file):
                            if generator_path != '' and venv_script_path != '':
                                if os.name == 'posix':
                                    subprocess.run([venv_script_path + "; " + PYTHON_CMD + " " + generator_path + " " + found_file + " " +output_directory + ";"], shell=True)
                                else:
                                    subprocess.run([venv_script_path], shell=True)
                                    subprocess.run([PYTHON_CMD, generator_path, found_file, output_directory], shell=True)
                            else:
                                raise FileNotFoundError("GMock Generator Script not Found!")

if __name__ == "__main__":
    import sys
    if len(sys.argv) != 4:
        print("Usage: python script.py <project_directory> <output_directory> <filepaths>")
        print("Given Args:")
        for arg in sys.argv:
            print(arg)
        sys.exit(1)
